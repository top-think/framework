...
class Console
{
    ...
    public function getDefinedCommands(array $config = [])
    {
        ...
        //读取组织指令集
        $groupPath = Container::get('env')->get('group_path');

        $groupModule = scandir($groupPath);
        foreach ($groupModule as $module) {
            if ('.' != $module && '..' != $module) {
                $path = $groupPath . $module . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR;
                if (is_dir($path) && is_file($path . 'module.json')) {
                    $file = $path . 'command.php';
                    if (is_file($file)) {
                        $groupCommands = include $file;

                        if (is_array($groupCommands)) {
                            $commands = array_merge($commands, $groupCommands);
                        }
                    }
                }
            }
