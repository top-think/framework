...
class Validate
{
     ...
     * @param array $scene 验证场景
     */
    public function __construct(array $rules = [], $message = [], $field = [], $scene = [])
    {
        $this->rule    = array_merge($this->rule, $rules);
        $this->message = array_merge($this->message, $message);
        $this->field   = array_merge($this->field, $field);
        $this->scene   = array_merge($this->scene, $field);
    }
     ...
     * @return Validate
     */
    public static function make($rules = [], $message = [], $field = [], $scene = [])
    {
        if (is_null(self::$instance)) {
            self::$instance = new self($rules, $message, $field, $scene);
        }
    ...
    {
        if (is_array($name)) {
            $this->scene = array_merge(array_filter($this->scene), $name);
        }if (is_null($fields)) {
            // 设置当前场景
